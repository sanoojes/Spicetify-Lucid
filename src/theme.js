function lucidMain(){if(!Spicetify.React || !Spicetify.ReactDOM || !Spicetify.Platform || !Spicetify.Player){setTimeout(()=>lucidMain(),100); return;}"use strict";
(() => {
  ...
  var PLACEHOLDER_IMAGE = 'url("https://cdn.jsdelivr.net/gh/sanoojes/Spicetify-Lucid@main/assets/placeholder.png")';
 ...
 `;
  ...
  function shallowCompareObjects(objA, objB) {
  ...
  var AppSettingsStore = class extends store_default {
    ...
    setIsCustomTopbarColor(isCustomColor) {
      this.setState((state) => ({
        ...state,
        topbar: {
          ...state.topbar,
  ...
  // extension/utils/changeRGBAlpha.ts
  function changeRGBAlpha(className, a = 0.5) {
...
body.hide-transparent-controls:after{display: none; content: none;}`;
  ...
  function intervalCall() {
    const intervalId2 = setInterval(() => mountControls(), 300);
    setTimeout(() => {
...
`;
  ...
  // extension/components/tour/tour.ts
  var GUIDED_TOUR_STYLE_ID = "guided-tour-styles";
  var GUIDED_TOUR_CSS = `
...
`;
  var GuidedTourElement = class extends HTMLElement {
    constructor() {
      super();
      this.steps = [];
    ...
    updateTooltip(step) {
      ...
      this.nextButton.textContent = this.currentStepIndex === this.steps.length - 1 ? "End Tour" : "Next";
    ...
    enablePointerEventsForInteractiveElements(step) {
      if (!this.tooltip || !this.targetElement) return;
      this.interactiveElements = [this.targetElement, this.tooltip];
      if (step.interactiveElementsSelector) {
        const elements = document.querySelectorAll(step.interactiveElementsSelector);
  ...
  var createButton = (className, textContent) => {
    const button = createElement("button", {
  ...
  // extension/tour/getTourSteps.ts
  var settingEventCb = () => {
  ...
  function getTourSteps(settings = setting_default.getState()) {
    ...
    const settingsPositionStep = settings.position === "nav" ? [
      {
  ...
  // extension/tour/guidedTour.ts
  var currentModal = null;
  var currentTourElement = null;
  var handleStartTour = () => {
  ...
  var createModalContent = () => {
    ...
    const startTourButton = modalContent.querySelector(".start-tour-btn");
    const skipTourButton = modalContent.querySelector(".skip-btn");
  ...
  var startTour = () => {
    try {
      ...
      const updateTourSteps = (settings) => {
        if (currentTourElement) currentTourElement.tourSteps = getTourSteps(settings);
      };
      updateTourSteps();
      setting_default.subscribe(updateTourSteps, "position");
      startModal();
      renderModal();
    } catch (e) {
        ...
        groups: [
          {
            ...
            fields: [
              field("isCusTopbarColor", {
                ...
                onChange: (state2) => {
                  settings.setIsCustomTopbarColor(state2);
                }
              }),
              field(
                "backgroundColor",
...
`;
  ...
  // extension/hooks/topbar.ts
  function mountTopbarStyles(settings = setting_default.getState().topbar) {
...
.search-searchCategory-SearchCategory{
    ...
    ${settings.isCustomColor ? `--top-bar-color: ${settings.bgColor.hex ? settings.bgColor.hex + alphaToHex(settings.bgColor.alpha) : `rgba(var(--clr-surface-1-rgb), ${settings.bgColor.alpha}%)`}` : ""} }`;
  }
  setting_default.subscribe((state) => {
    mountTopbarStyles(state.topbar);
  }, "topbar");

  // extension/hooks/pages.ts
  function mountPlaylistModalObserver() {
    waitForElement([".Root"], ([root]) => {
      if (!root) return;
      const rootObserver = new MutationObserver(() => {
        const sourceDiv = root.querySelector(".FP_XXx0FMQPJEu3WzfpM");
        if (!sourceDiv) return;
