import { isCustomControls, isLightModeEnabled, isSpotifyV46Above, isWindowsPlatform } from "@/constants/constants";
		...
		const setTopBarStyles = () => {
			if (isWindowsPlatform) {
				...
				if (!isCustomControls && !isLightModeEnabled) {
					const normalHeight = controlHeight || (isSpotifyV46Above ? 32 : 64);
					const controlTop = isSpotifyV46Above
						? (controlHeight / windowZoom - Math.min(32 / windowZoom, controlHeight / windowZoom)) / 2
						: 0;
