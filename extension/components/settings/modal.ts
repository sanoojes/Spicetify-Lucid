...
export class FloatingModal extends HTMLElement {
  private _modalElem: HTMLDivElement;
  private _bodyElem: HTMLDivElement;
  private _scrollElem: HTMLDivElement;
  private _closeBtn: Button;
  private _githubBtn: Button;
  private _discordBtn: Button;
  private _headerElem: HTMLDivElement;
  private _bgElem: HTMLDivElement;
  ...
  constructor() {
    ...
    this._headerElem = createElement('div', {
      className: 'modal-header-container',
    });
    const header = createElement('h2', {
      className: `modal-header ${getTextClass('title-small')}`, // Corrected template literal syntax
      textContent: 'Lucid Settings',
      id: 'modal-header',
    });
    ...
    this._headerElem.append(header, buttons);
    this._modalElem.appendChild(this._headerElem);
  ...
  private handlePointerMove = (e: PointerEvent) => {
    ...
    this._modalElem.style.transform = `translate3d(${newLeft}px,${newTop}px,0px)`; // Corrected template literal syntax
  ...
  public makeFloating() {
    ...
    this._headerElem.addEventListener('pointerdown', this.handlePointerDown);
  }
  ...
  public removeFloating() {
    ...
    this._headerElem.removeEventListener('pointerdown', this.handlePointerDown);
  }
...
customElements.define('lucid-modal', FloatingModal);
