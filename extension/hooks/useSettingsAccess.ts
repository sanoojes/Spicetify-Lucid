...
const useSettingsAccess = () => {
	...
	const element = useRef<Spicetify.Menu.Item | Spicetify.Topbar.Button | null>(null);
	...
	useEffect(() => {
		...
		if (settingAccessPosition === "nav") {
			if (
				...
				!(element.current instanceof HTMLButtonElement)
			) {
		...
		return () => {
			...
			document
				.querySelector(".Root__globalNav .main-actionButtons div:has(button[aria-label='Lucid Settings'])")
				?.remove();
		};
