...
const openModal = () => {
  // add the same css as settings page
  // unload linkElem ??
  ...
  waitForGlobal(() => Spicetify?.PopupModal).then((modal) => {
    ...
    // waitForElements('generic-modal').then((e) => {
    //   e.classList.add('lucid-settings');
    // });
  });

  // linkElem.addEventListener('error', display); // in case the link changes
  // linkElem.addEventListener('load', display);
};

async function addSettings(position = appStore.getState().settingModal.accessPoint) {
  await addSettingsEntry(position, openModal);
}
appStore.subscribe((state) => state.settingModal.accessPoint, addSettings);

async function addSettingsEntry(
  position = appStore.getState().settingModal.accessPoint,
  openModal: () => void
) {
  // Avoid Duplicate Nav button
  if (
    position === 'nav' &&
