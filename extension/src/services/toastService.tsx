...
import React, { useState, type FC, useCallback, ReactNode } from "react";
...
const ToastService = (() => {
  ...
  const ToastContainer: FC = () => {
    ...
    const addToastInternal = useCallback(
      (message: ReactNode, isError = false) => {
        const id = toastId++;
        setToasts((prev) => [
          ...prev,
      ...
      <div className='toast-container'>
        {toasts.map((toast) => (
          <div
            ...
            style={{
              backgroundColor: `${
                toast.isError ? "rgba(200,20,20,0.5)" : "rgba(20,20,20,0.5)"
              }`,
            }}
          >
            {toast.message}
  ...
  return {
    ToastContainer,
    addToast: (message: ReactNode, isError?: boolean) =>
